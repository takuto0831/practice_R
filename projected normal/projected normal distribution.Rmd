...
[stan 確率分布を自分で定義する](http://norimune.net/2810)
...
fit <- stan("C:/Users/SHIO-160412-4/Desktop/practice_R/projected normal/script/test.stan",
...
```{r}
fit.coda<-mcmc.list(lapply(1:ncol(fit),function(x) mcmc(as.array(fit)[,x,1:3])))
plot(fit.coda)
```

```{r}
fit.coda<-mcmc.list(lapply(1:ncol(fit),function(x) mcmc(as.array(fit)[,x,4:6])))
plot(fit.coda)
```

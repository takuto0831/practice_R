...
projected_normal_circular <- function(theta,mu,Sigma){
  ...
  tmp = B/sqrt(A)
  p = 1/(2*pi*A*sqrt(det(Sigma))) * exp(C)*(1 + tmp*dnorm(tmp,0,1)/pnorm(tmp,0,1))
  return(p)
}
...
# 違う論文バージョン
# よくわからない
projected_normal_circular_2 <- function(theta,mu,Sigma){
  phi2 = dmvnorm(mu %>% as.vector(),c(0,0),Sigma)
  a = 1/sqrt(det(Sigma))
  C = a^2 * (t(u) %*% solve(Sigma) %*% u)
  D = a^2 * (1/sqrt(C)) * (t(u) %*% solve(Sigma) %*% mu)
  ans = phi2/C + a*D
  }
